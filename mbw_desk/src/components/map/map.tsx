...
function HistoryMap({ options, onLoad, HistoryIndex }) {
  ...
  const defaultOptions = {
    ...
    icon: {
      ...
      start: `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="44px" height="44px" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fillRule:evenodd; clipRule:evenodd">
  ...
  const initializeMap = () => {
    try {
      ...
      _map.on("load", () => {
        try {
          if (
            !_options.objectId ||
            _options.objectId === "" ||
            _options.objectId === "null" ||
            !_options.from_time ||
            _options.from_time === "" ||
            _options.from_time === "null" ||
            !_options.to_time ||
            _options.to_time === "" ||
            _options.to_time === "null"
          ) {
            onLoad(null);
            return;
          }
      ...
      const setMap = async (from_time, to_time, pageNumber, pageSize) => {
        ...
        if (
          detailsHis.length &&
          detailsHis[0].coordinates &&
          _options.animation.animate
        )
          _map.easeTo({
            center: detailsHis[0].coordinates,
            duration: 100,
            zoom: 16,
          });
        setTimeout(() => {
      ...
      const initDataToMap = async (data) => {
        ...
        data.forEach((item) => {
          ...
          if (item.type === "move") {
            ...
            } else LineSource.features.push(feature);
          } else {
        ...
        try {
          ...
          if (_map.getSource(`ek-tracking-his-track-source`)) {
            ...
            _map.on("mouseenter", `ek-tracking-his-track-point`, (e) => {
              ...
              let color = `default`;
              if (features.properties.type !== "stop")
                color = features.properties.type;

              if (popupHis.current) popupHis.current.remove();
              popupHis.current = new maplibregl.Popup({
                className: "ek-tracking-his-popup",
                closeButton: false,
                anchor: "bottom",
              })
          ...
          if (_map.getSource(`ek-tracking-his-check-source`)) {
            ...
            _map.on("click", `ek-tracking-his-check-clusters`, async (e) => {
              ...
              const zoom = await _map
                .getSource(`ek-tracking-his-check-source`)
                ...
                .setHTML(
                  `
                                ...
                                <div class="ek-tracking-his-popup-info">
                                    ...
                                    <span>${formatTimestamp(
                                      selectedFeatures[0].properties.startTime
                                    )}</span>
                                </div>
                                ${
      ...
      function drawPath() {
        ...
        } else _map.getSource("LocationHistory").setData(fcLocationHistory);
      }
      async function addCar() {
        ...
        } else _map.getSource("locationMarker").setData(locationMarker);
      }
    } catch (error) {
    ...
    function computeData(gpxData) {
      ...
      for (let i = 0; i < gpxData.segments.length; i++) {
        ...
        for (let j = 1; j < segment.length; j++) {
          ...
          if (
            loc1Time.getDate() === loc2Time.getDate() &&
            loc1Time.getMonth() === loc2Time.getMonth() &&
            loc1Time.getFullYear() === loc2Time.getFullYear()
          ) {
            d = getSegDistance(
              locData1.loc[1],
              locData1.loc[0],
          ...
          if (
            locData1.time &&
            locData2.time &&
            locData1.time !== locData2.time
          ) {
            const { speed, time } = getSegSpeed(
              d,
              locData1.time,
              locData2.time
            );
      ...
      for (let i = 0; i < segmentData.length; i++) {
        ...
        if (seg.time) {
          ...
          if (seg.speed && seg.filteredSpeed) {
            ...
            if (
              seg.filteredSpeed > result.maxSpeed &&
              seg.filteredSpeed < 200
            ) {
  ...
  useEffect(() => {
    ...
    if (HistoryIndex !== null) {
      ...
      if (detail.type === "move") {
        setTimeout(async () => {
          HisMap.setPaintProperty(
            `ek-tracking-his-LineString`,
            "line-color",
            "#b5b5b5"
          );
          HisMap.setPaintProperty(
          ...
          HisMap.setLayoutProperty(
            `ek-tracking-his-LineString-arrow`,
            "visibility",
            "none"
          );
          ...
          if (
            !HisMap.getImage("icon_arrow") &&
            !HisMap.hasImage("icon_arrow")
          ) {
            var svg_arrow =
            ...
            icon_arrow.src =
              "data:image/svg+xml;charset=utf-8;base64," + btoa(svg_arrow);
          }
      ...
      try {
        ...
        if (HisMap) {
          HisMap.setPaintProperty(
            `ek-tracking-his-LineString`,
            "line-color",
            _options.route.lineCorlor
          );
          HisMap.setPaintProperty(
            `ek-tracking-his-LineString`,
            "line-opacity",
            _options.route.lineOpacity
          );
          HisMap.setLayoutProperty(
            `ek-tracking-his-LineString-arrow`,
            "visibility",
    ...
    <>
      <div ref={mapContainer} id="ekmap-tracking-his-map">
        {Controls.ready !== false && (
          <Animate_And_Controls
            map={HisMap}
            segmentData={Controls.Data}
        ...
        <MapLegend />
        {Stage.date !== null && Stage.time !== null && (
          <div className="ekmapplf_tracking-map-legend-his">
            <Flex
              vertical="true"
              align="center"
              justify="center"
            ...
            <Flex
              ...
              justify="center"
              wrap="wrap"
              style={{ width: "49%", fontWeight: 600, textAlign: "center" }}>
