<template>
	<Card
		:title="$t('App_Profile')"
		:subtitle="$t('MarketplaceAppProfile_content_1')"
	>
		...
		<div class="mt-8 flex justify-between">
			<p class="text-lg font-semibold">{{ $t('Published_Versions') }}</p>
			<Button
				icon-left="plus"
				@click="
					() => {
		...
		>
			<template v-slot:body-content>
				<FormControl :label="$t('App_Title')" v-model="app.title" />
...
export default {
	...
	methods: {
		...
		notifySuccess() {
			notify({
				title: this.$t('MarketplaceAppProfile_content_2'),
				icon: 'check',
				color: 'green'
			});
		}
