...
def export_file(res):
    try:
            ...
            if log_trips.get("details"):
                for log in log_trips.get("details"):
                    ...
                    if date in data[d]:
                        ...
                        checkin_cuoingay = data[d][date]["checkin_cuoingay"]
                        if checkin_daungay <= time <= checkin_cuoingay and log.get("type") == "move":
                            data[d][date]["total_distance"] = data[d][date]["total_distance"] + float(
                                log.get("distance"))
                        # cap nhat doanh thu va so don hang theo key employee_code va date
                        if d in data_si and date in data_si[d]:
                            data[d][date]["total_donhang"] = data_si[d][date]['so_don_hang']
        ...
        report = [
            {**entry, "total_distance": round(entry["total_distance"] / 1000, 2), "checkindate": entry["checkin_daungay"].strftime("%d-%m-%Y"),
             "checkin_daungay": entry["checkin_daungay"].strftime("%H:%M"),
             "checkin_cuoingay": entry["checkin_cuoingay"].strftime("%H:%M"),
             "total_checkin": entry['checkin_sang'] + entry['checkin_chieu'],
             }
            for d in data for entry in data[d].values()
        ]
        if is_excel:
            return {
                "data": report,
                "filters": {"from_date": datetime.strptime(from_date, "%Y-%m-%d %H:%M:%S").strftime("%d-%m-%Y"),
                            "to_date": datetime.strptime(to_date, "%Y-%m-%d %H:%M:%S").strftime("%d-%m-%Y"),
                            "employee": employee,},
            }
        else:
