...
def get_data_when_grouped_by_invoice(columns, gross_profit_data, filters, group_wise_columns, data):
	...
	for src in gross_profit_data.si_list:
		if bool(filters.get("brand")) and filters.get("brand") is not None:
			if src.get('brand') != filters.get("brand"):
				continue
		row = frappe._dict()
...
def get_data_when_not_grouped_by_invoice(gross_profit_data, filters, group_wise_columns, data):
	...
	for src in gross_profit_data.grouped_data:
		if not src.get('brand'):
			brand = gross_profit_data.get_bundle_item_details(src.get('item_code'))
			src['brand'] = brand[3]
		if bool(filters.get("brand")) and filters.get("brand") is not None:
			if src.get('brand') != filters.get("brand"):
				continue
	...
	column_map = frappe._dict(
		{
			...
			"brand": {"label": _("Brand"), "fieldtype": "Link", "options": "Brand", "width": 170},
			"description": {
				"label": _("Description"),
