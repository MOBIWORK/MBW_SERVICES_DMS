...
def get_gl_entries(filters, accounting_dimensions):
	...
	for gl_entry in gl_entries:
		payment_entry = frappe.get_doc(gl_entry.voucher_type, gl_entry.voucher_no)
		gl_entry["payment_type"] = payment_entry.get("payment_type")
		gl_entry["custom_note"] = payment_entry.get("custom_note")

	if filters.get("payment_type") and filters.get("payment_type") != "All":
		gl_entries = pydash.filter_(gl_entries, {"payment_type": filters.get("payment_type")})
...
def get_columns(filters):
	...
	if filters.get("show_remarks"):
		columns.extend([{"label": _("Remarks"), "fieldname": "remarks", "width": 400,"hidden": 1,}])
