...
def get_data(filters):
	query = f"""SELECT si.name, t.name AS territory, si.posting_date, si.status, si.customer, si.customer_name, si.company, st.sales_person,
				sii.brand, sii.item_code, sii.item_name, sii.qty, sii.uom, sii.amount, sii.custom_total_litres_base_on_1_item
				FROM `tabSales Invoice` si
				JOIN `tabSales Invoice Item` sii ON sii.parent = si.name
				JOIN `tabTerritory` t ON si.territory = t.name AND t.parent_territory = 'Z2 Vĩnh Long'
