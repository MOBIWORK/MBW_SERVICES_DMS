...
from mbw_dms.api.common import exception_handle, gen_response, get_sales_group_child_v2, get_language, get_user_id, get_employee_by_user, time_now_utc, null_location, get_sales_group_child, weekday
...
def get_customer_router(data):
    try:     
        ...
        # Nếu là admin thì lấy tất cả tuyến hoạt động, nếu là nhân viên chỉ lấy nhân viên chăm sóc tuyến đấy
        user_id = get_user_id()
        ...
        # Dịch vụ lấy khách hàng hiển thị cho map: chỉ tuyến hôm nay và tuần này
        if view_mode == "map":
        ...
        for customer in detail_customer:
            ...
            customer["is_checkin"] = customer["customer_code"] in list_checkin_code or False
...
def get_customers_import(data):
    try:
        ...
        fields_customer = ["name", "customer_primary_address"
            ,"customer_code", "customer_location_primary", "mobile_no"
            ,"customer_name", "UNIX_TIMESTAMP(custom_birthday) as birthday"
            ]
    ...
    try:
        ...
        for key_router, value in body.items():
            if isinstance(body[key_router], list):
                ...
                if key_router not in field_validate :
                    return gen_response(406, 
                                        f"Field {key_router} not validate",None)
...
def update_routers(body):
    try:
        ...
        if len(name) == 1:
            frappe.db.set_value("DMS Router", name[0], {field_name: value})
        else:
            name_arr = tuple(name)
            frappe.db.sql(f"""
